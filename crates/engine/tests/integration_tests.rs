...
async fn test_full_memory_lifecycle() {
    ...
    let bundle = engine.recall("Rust ownership").await.expect("Failed to recall");
    assert!(!bundle.is_empty(), "Should find at least one memory");
    assert!(bundle.memories.iter().any(|m| m.summary.contains("Rust")), "Should find Rust-related memory");
}
...
async fn test_semantic_search_ranking() {
    ...
    let test_data = vec![
        ...
        ("Vector databases store embeddings for semantic search.", "Vector DB"),
    ...
    for (content, summary) in &test_data {
        engine.remember(*content, *summary, MemoryTrigger::Insight).await
            .expect("Failed to store memory");
    }

    // Query for vector-related content
    let bundle = engine.recall("vector database search").await.expect("Failed to recall");
...
async fn test_injection_blocked_on_store() {
    ...
    assert!(result.clean_text.contains("[filtered]"), "Should be filtered");
}

/// Pruner evaluates memories correctly
#[tokio::test]
async fn test_pruner_lifecycle() {
    use rememnemosyne_engine::{MemoryPruner, PrunerConfig};

    let pruner = MemoryPruner::default();

    let high = MemoryArtifact::new(
        MemoryType::Semantic, "high", "high content",
        vec![0.1; 1536], MemoryTrigger::Insight,
    ).with_importance(Importance::High);
    assert!(pruner.should_keep(&high), "High importance should be kept");

    let low = MemoryArtifact::new(
        MemoryType::Semantic, "low", "low content",
        vec![0.1; 1536], MemoryTrigger::UserInput,
    ).with_importance(Importance::Low);
    assert!(pruner.should_keep(&low), "Fresh low importance should be kept");
}
...
async fn test_context_bundle_relevance() {
    ...
    let memory = MemoryArtifact::new(
        MemoryType::Semantic, "test", "test content",
        vec![0.1; 1536], MemoryTrigger::Insight,
    );
    let id = memory.id;
